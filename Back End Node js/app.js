...
app.put('/updatepoll/:id', auth, async (req, res) => {
    ...
    try {
        const poll = await Poll.findByIdAndUpdate(req.params.id, { title, description, startdate, enddate, optionone, optiontwo });
...
app.put('/updatevote/:id', auth, async (req, res) => {
    ...
    try {
        await Poll.findByIdAndUpdate(req.params.id, { votes });
        res.status(200).send({
            "status": "success",
